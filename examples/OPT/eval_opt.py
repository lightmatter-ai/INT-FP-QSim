...
def batch_process_func(model, inputs):
    ip = {k: v.cuda() for k, v in inputs.items()}
    model(**ip)


def do_static_calibration(
    model, dataloader, num_bits, method, percentile, num_samples=5, calib_inputs=True
):
    # Make sure to use a quantizer that takes in scale
    # For use with: quantize_to_int or quantize_to_fp
    if calib_inputs:
        calibrate_inputs(
            model.cuda(),
            num_bits,
    ...
    calibrate_weights(model, 4, False, method="max", perchannel=True)
